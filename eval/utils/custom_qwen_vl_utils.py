...
def calculate_video_frame_range(
    ...
    # Process start frame
    if "video_start_fr" in ele and ele["video_start_fr"] is not None:
        ...
        start_frame = 0
    # Process end frame
    if "video_end_fr" in ele and ele["video_end_fr"] is not None:
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    if isinstance(ele["video"], str):
        ...
        try:
            ...
            logger.warning(f"video_reader_backend {video_reader_backend} error, use decord as default, msg: {e}")
            video, video_metadata, sample_fps = VIDEO_READER_BACKENDS["decord"](ele)
    else:
