...
class TextRetriever:
    ...
    def load_feats(self, queryfeats_path, textfeats_path):
        print(f"Loading TextRetriever from {textfeats_path}...")
    ...
    def retrieve(self, query_id, top_k: int = 5, long_text: bool = False):
        ...
        if not long_text:
            top_k_text_ids = [self.textids[rank] for rank in rankings[0][:top_k]]
        else:
