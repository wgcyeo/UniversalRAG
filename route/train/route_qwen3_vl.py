...
class Qwen3VLMultiLabelClassifier(nn.Module):
    ...
    def _get_base_model(self):
        ...
        # Qwen3VLForConditionalGeneration.forward always applies lm_head.
        # The router only needs hidden states, so call the base VL model.
...
class Qwen3VLMultiLabelRouter:
    """Router using fine-tuned Qwen3-VL multi-label classifier."""
    ...
    def __init__(
        ...
        self.model = Qwen3VLMultiLabelClassifier(
            backbone=backbone,
    ...
    def _build_inputs(self, query: str, query_image: str = None):
        return self._build_batch_inputs([query], query_images=[query_image])

    def _build_batch_inputs(self, queries: list, query_images: list = None):
        ...
        for query, query_image in zip(queries, query_images):
    ...
    def _to_model_inputs(self, inputs: dict):
        return {
    ...
    def _predict_batch(
        ...
        inputs = self._build_batch_inputs(queries, query_images=query_images)
        inputs = self._to_model_inputs(inputs)
    ...
    def route_batch(
        ...
        for i in tqdm(range(0, len(indexed_items), batch_size), desc="Routing queries"):
            ...
            batch_queries = [query for _, query, _ in batch_items]
            batch_images = [image for _, _, image in batch_items]
...
def main():
    parser = argparse.ArgumentParser(description="Route queries using trained Qwen3-VL multi-label model")
    ...
    parser.add_argument("--output-dir", type=str, default="route/results/qwen3_vl_2b", help="Directory to save predictions when processing a directory")
    ...
    router = Qwen3VLMultiLabelRouter(
        args.model_path,
